...
class Train(Dataset):
	...
	def __getitem__(self, index):
		...
		if self.mode == 'x-y':
			...
			img2 = imgs[1]
			lb2 = lbs[1]
			affs = gen_affs(lb1, lb2, dir=0, shift=self.shift)
			affs = affs[np.newaxis, ...]
		elif self.mode == 'x-y-z':
			img2 = imgs[1]
			lb2 = lbs[1]
...
if __name__ == '__main__':
	...
	np.random.seed(seed)
	random.seed(seed)
	cfg_file = 'seg_isbi_data80_c16_xyz2_wz2_mse.yaml'
	...
	for i in range(0, 50):
		...
		im_cat = show_twoImage(tmp_data, weightmap)
