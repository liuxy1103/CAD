...
def gen_affs_mutex_single(map1, map2, shift=[0,0,0], padding=True, background=False):
...
def gen_affs_mutex_multi(map1, map2, shift=[[0,0,0]], padding=True, background=False):
    affs = []
    for k in shift:
        affs_single = gen_affs_mutex_single(map1, map2, shift=k, padding=padding, background=background)
        affs.append(affs_single)
    ...
    for shift_k in shift:
        ...
        for i in range(labels.shape[0]):
            if shift_k[0] != 0:
                if i == 0:
                    if padding:
                        temp = gen_affs_mutex_single(labels[0], labels[1], shift=shift_k, padding=padding, background=background)
                    else:
                        temp = np.zeros_like(labels[0], dtype=np.float32)
                else:
                    temp = gen_affs_mutex_single(labels[i-1], labels[i], shift=shift_k, padding=padding, background=background)
            else:
                temp = gen_affs_mutex_single(labels[i], labels[i], shift=shift_k, padding=padding, background=background)
            affs_k.append(temp)
        affs.append(affs_k)
