...
def loop(cfg, cfg_3d, train_provider, valid_provider, train_provider_3d, valid_provider_3d, model, model_3d, criterion, optimizer, iters, writer):
    ...
    while iters <= cfg.TRAIN.total_iters:
        ...
        # loss_3d, pred_3d = embedding_loss_norm_multi(embedding_3d, target_3d, weightmap_3d, criterion, affs0_weight=cfg_3d.TRAIN.affs0_weight,shift=cfg_3d.DATA.shift_channels)
        ...
        #pred_3d: (2,12,16,160,160),pred_3d[0,0,0]是padding的可以从下面开始考虑
        # inputs_tmp = torch.zeros((inputs.shape[:])).cuda()
        shift = 1
        ...
        for z in range(inputs_3d.shape[2]-1):
            ...
            if cfg.TRAIN.interaction:
                ...
                # embedding_3d_tmp[:,:,z] = embedding_3d_tmp[:,:,z] - embedding_3d_tmp[:,:,z] 
                # embedding_3d_tmp[:,:,z] += embedding1_tmp
                # embedding_3d_tmp[:,:,z+1] = embedding_3d_tmp[:,:,z+1] - embedding_3d_tmp[:,:,z+1]
                # embedding_3d_tmp[:,:,z+1] += embedding2_tmp
                # embedding_3d_tmp[:,:,z] = embedding1_tmp
                # embedding_3d_tmp[:,:,z+1] = embedding2_tmp
...
# valid
        if cfg_3d.TRAIN.if_valid:
            if iters % cfg.TRAIN.save_freq == 0 or iters == 1:
                device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
                model.eval()
                dataloader_3d = torch.utils.data.DataLoader(valid_provider_3d, batch_size=1, num_workers=0,
                                                shuffle=False, drop_last=False, pin_memory=True)
                losses_valid = []
                for k, batch in enumerate(dataloader_3d, 0):
                    inputs, target, weightmap = batch
                    inputs = inputs.cuda()
                    target = target.cuda()
                    weightmap = weightmap.cuda()
                    with torch.no_grad():
                        embedding = model_3d(inputs)
                    ...
                    if cfg_3d.TRAIN.embedding_mode == 1:
                        tmp_loss, pred = embedding_loss_norm1(embedding, target, weightmap, criterion, affs0_weight=cfg_3d.TRAIN.affs0_weight)
                    elif cfg_3d.TRAIN.embedding_mode == 5:
                        tmp_loss, pred = embedding_loss_norm5(embedding, target, weightmap, criterion, affs0_weight=cfg_3d.TRAIN.affs0_weight)
                    else:
                        raise NotImplementedError
                    shift = 1
                    ...
                    pred = F.relu(pred)
                    losses_valid.append(tmp_loss.item())
                    valid_provider_3d.add_vol(np.squeeze(pred.data.cpu().numpy()))
                epoch_loss = sum(losses_valid) / len(losses_valid)
                out_affs = valid_provider_3d.get_results()
                gt_affs = valid_provider_3d.get_gt_affs().copy()
                gt_seg = valid_provider_3d.get_gt_lb()
                valid_provider_3d.reset_output()
                out_affs = out_affs[:3]
                # gt_affs = gt_affs[:, :3]
                ...
                # whole_f1 = 1 - f1_score(gt_affs.astype(np.uint8).flatten(), out_affs.astype(np.uint8).flatten())
                whole_f1 = f1_score(1 - gt_affs.astype(np.uint8).flatten(), 1 - out_affs.astype(np.uint8).flatten())
                print('model_3d-%d, valid-loss_3d=%.6f, MSE-loss_3d=%.6f, BCE-loss_3d=%.6f, F1-score_3d=%.6f, VOI-waterz_3d=%.6f, ARAND-waterz_3d=%.6f, VOI-lmc_3d=%.6f, ARAND-lmc_3d=%.6f' % \
                    (iters, epoch_loss, whole_mse, whole_bce, whole_f1, voi_sum_waterz, arand_waterz, voi_sum_lmc, arand_lmc), flush=True)
                writer.add_scalar('valid/epoch_loss_3d', epoch_loss, iters)
                writer.add_scalar('valid/mse_loss_3d', whole_mse, iters)
                writer.add_scalar('valid/bce_loss_3d', whole_bce, iters)
                writer.add_scalar('valid/f1_score_3d', whole_f1, iters)
                writer.add_scalar('valid/voi_waterz_3d', voi_sum_waterz, iters)
                writer.add_scalar('valid/arand_waterz_3d', arand_waterz, iters)
                writer.add_scalar('valid/voi_lmc_3d', voi_sum_lmc, iters)
                writer.add_scalar('valid/arand_lmc_3d', arand_lmc, iters)
                f_valid_txt.write('model_3d-%d, valid-loss_3d=%.6f, MSE-loss_3d=%.6f, BCE-loss_3d=%.6f, F1-score_3d=%.6f, VOI-waterz_3d=%.6f, ARAND-waterz_3d=%.6f, VOI-lmc_3d=%.6f, ARAND-lmc_3d=%.6f' % \
                                (iters, epoch_loss, whole_mse, whole_bce, whole_f1, voi_sum_waterz, arand_waterz, voi_sum_lmc, arand_lmc))
                f_valid_txt.write('\n')
                f_valid_txt.flush()
                torch.cuda.empty_cache()
        ...
        if iters % cfg.TRAIN.save_freq == 0:
            ...
            torch.save(states, os.path.join(cfg.save_path, 'model_3d-%06d.ckpt' % iters))
            print('***************save 3D modol, iters = %d.***************' % (iters), flush=True)
    f_loss_txt.close()
    f_valid_txt.close()
...
if __name__ == "__main__":
    ...
    if args.mode == 'train':
        ...
        # model_3d,  _ = resume_params_3d(cfg_3d, model_3d,  cfg.TRAIN.resume_3d)
