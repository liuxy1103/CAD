...
def embedding2rgb(embedding):
    assert len(embedding.shape) == 3, 'the shape of embedding must be: [C, H, W]'
    C, H, W = embedding.shape
    pca = PCA(n_components=3)
    embedding = np.transpose(embedding, [1, 2, 0]) #特征放在最后一个维度（H，W, C）
    embedding = embedding.reshape(-1, C)
    new_emb = pca.fit_transform(embedding) 
    new_emb = new_emb.reshape(H, W, 3)
    # Norm
    new_emb = (new_emb - np.min(new_emb)) / np.max(new_emb)
    new_emb = (new_emb * 255).astype(np.uint8)
    return new_emb

def show_embedding(embedding1, embedding2, inputs, k, embedding_img_path):
    embedding1 = np.squeeze(embedding1.data.cpu().numpy())
    embedding2 = np.squeeze(embedding2.data.cpu().numpy())
    inputs = np.squeeze(inputs.data.cpu().numpy())
    inputs = (inputs * 255).astype(np.uint8)
    img1 = inputs[0]
    img1 = img1[:,:,np.newaxis]
    img1 = np.repeat(img1, 3, 2)
    img2 = inputs[1]
    img2 = img2[:,:,np.newaxis]
    img2 = np.repeat(img2, 3, 2)
    emd1 = embedding2rgb(embedding1)
    emd2 = embedding2rgb(embedding2)
    cat1 = np.concatenate([img1, img2], axis=1)
    cat2 = np.concatenate([emd1, emd2], axis=1)
    im_cat = np.concatenate([cat1, cat2], axis=0)
    Image.fromarray(im_cat).save(os.path.join(embedding_img_path, str(k).zfill(4)+'.png'))
